      ...
      createUserWithEmailAndPassword(auth, email, password)
        ...
        .catch((err: any) => {
          if (err.message === 'Firebase: Error (auth/email-already-in-use).') {
            Alert.alert(
              'The email entered has already been used, please try another one.'
            );
          }
        });
    } else {
    ...
    <>
      <View className="flex-1 bg-white px-8 pt-8 justify-center">
        <SafeAreaView className="flex justify-center items-center mb-6">
          <Text className="font-extrabold text-4xl text-sky-600">SIGN UP</Text>
        </SafeAreaView>
        <View className="form space-y-2">
          ...
          />
          <TouchableOpacity
            className="py-3 bg-sky-600 rounded-2xl"
