      ...
      getDoc(userDocRef)
        .then(docSnap => {
          ...
          if (docSnap.exists()) {
            ...
            setDuration(
              userData.goals.duration
                ? Math.floor(userData.goals.duration / 60)
                : 10000 / 60
            );
    ...
    if (user) {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          const userData = docSnap.data();
          const lastUpdated =
            userData.lastUpdated || Timestamp.fromDate(new Date(0)); // Default to a very old date as a Timestamp
      ...
      {isLoading ? (
        ...
        <View className="flex-1 flex-col items-center m-4 justify-between pb-8">
          ...
          <View className="w-80 h-80 bg-white border-0.5 rounded-lg flex flex-col py-8 px-6 justify-between">
            <GoalSection
              title="Step (steps)"
              currentValue={walking}
              onMinus={() => walking && setWalking(walking - 1)}
              onPlus={() => walking && setWalking(walking + 1)}
            />
            <GoalSection
              title="Distance (km)"
              currentValue={distance}
              onMinus={() => distance && setDistance(distance - 1)}
              onPlus={() => distance && setDistance(distance + 1)}
            />
            <GoalSection
              title="Duration (mins)"
              currentValue={duration}
              onMinus={() => duration && setDuration(duration - 1)}
              onPlus={() => duration && setDuration(duration + 1)}
            />
          </View>
          <TouchableOpacity
            className="py-2 bg-blue-brand rounded-full w-64 h-12"
