...
const Home = () => {
  ...
  async function queryStep() {
    ...
    if (user && user.uid) {
      const q = query(
        collection(db, 'exercise_walking'),
        where('uid', '==', user.uid)
      );
      ...
      querySnapshot.forEach(doc => {
        if (
          doc.data().start_date.toDate().toDateString() ===
    ...
    <View className="flex flex-1 items-center w-screen h-screen ">
      ...
      <View className="w-full flex flex-row flex-even items-center justify-center">
        ...
        <Card containerStyle={{ width: '25%', borderColor: 'transparent' }}>
          ...
          <View className="flex justify-center items-center">
            <Text>
              {cyclingDistance}/{cyclingDistanceGoal}
            </Text>
          </View>
        </Card>
        <Card containerStyle={{ width: '25%', borderColor: 'transparent' }}>
          ...
          <View className="flex justify-center items-center">
            <Text>
              {cyclingDuration}/{cyclingDurationGoal}
            </Text>
          </View>
        </Card>
      </View>
